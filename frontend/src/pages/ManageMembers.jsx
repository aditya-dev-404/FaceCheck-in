...
import { MdAdd, MdCancel, MdCheck, MdDeleteOutline, MdEdit, MdFlag, MdGroup, MdPersonOff, MdPersonOutline } from "react-icons/md";
...
const ManageMembers = () => {
  ...
  const [editForm, setEditForm] = useState({ name: "", email: "", category: "" });
  ...
  const startEdit = (member) => {
    ...
    setEditForm({ name: member.name, email: member.email, category: member.category || "" });
    setError("");
  };
  ...
  const saveEdit = async (id) => {
    setError("");
    try {
      await axiosInstance.patch(`/users/${id}`, editForm);
      setEditingId(null);
      fetchMembers();
    } catch (err) {
  ...
  return (
    <div className="clay-page">
      <div className="clay-page-header">
        <div><p className="text-sm font-medium text-primary">Team management</p><h1 className="mt-1 text-2xl font-semibold text-text">Manage members</h1></div>
        <Link
          to="/add-member"
      ...
      <div className="overflow-x-auto rounded-clay bg-surface shadow-clay-sm">
                ...
                <tr key={member._id} className={!member.isActive || member.isFlagged ? "bg-surface-raised" : ""}>
                  {editingId === member._id ? (
                    <>
                      ...
                      <td className="px-4 py-2 text-text-muted">—</td>
                      <td className="px-4 py-2"><div className="flex min-w-40 flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => saveEdit(member._id)}
                          className="clay-btn-primary px-2.5 py-1.5 text-xs"
                        >
                          <MdCheck aria-hidden="true" /> Save
                        </button>
                        <button
                          type="button"
                          onClick={cancelEdit}
                        ...
                        >
                          <MdCancel aria-hidden="true" /> Cancel
                        </button>
                      </div></td>
                    </>
                  ) : (
                    <>
                      ...
                      <td className="px-4 py-2"><span className={`clay-badge ${member.isEnrolled ? "text-success" : ""}`}>{member.isEnrolled ? "Enrolled" : "Pending"}</span></td>
                      <td className="px-4 py-2">
                        <span
                      ...
                      <td className="px-4 py-2"><div className="flex min-w-52 flex-wrap gap-2">
                        <button
                          type="button"
                        ...
                        {member.isFlagged && (
                          <button
                            ...
                            className="clay-action-danger px-2.5 py-1.5 text-xs"
                          >
                            Unflag
                          </button>
                        )}
